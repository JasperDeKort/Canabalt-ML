...
# sizes of input, output's, batches and test
batch_size = 100
testsize = 1000
n_classes = 2
plusfeat = 3
...
def model(input_data, input_data2, filter1, filter2 , layer_keep, weights1, weights2):
    ...
    with tf.name_scope("hidden_3_dense"):
        ...
        mergeflat = tf.concat([pool2_flat, input_data2],1)
        layer3 = tf.nn.relu(tf.matmul(mergeflat, weights1))
...
def create_graph(epsilon, beta1, beta2, learning_rate,filtersize, l1_outputchan,
    ...
    return prediction, optimizer, acc_op, conf_mat, cost, x, x2, y, layer_keep_holder
    
def train_neural_network(x_train, x2_train, y_train, x_test, x2_test, y_test,x_img,prediction, optimizer,
                         acc_op, conf_mat, cost, x, x2, y, layer_keep_holder,log_folder):
    ...
    with tf.Session() as sess:   
        ...
        for epoch in range(hm_epochs):
            ...
            for start, end in zip(range(0, len(x_train), batch_size), range(batch_size, len(x_train)+1, batch_size)):
                ...
                feed_dict = {x: batch_x, x2: batch_x2, y: batch_y,layer_keep_holder: layer_keep}
                _, c = sess.run([optimizer, cost], feed_dict)
                epoch_loss += c
                i += end - start
                # generate test and train data summaries every 10000 datapoints
                if start % 10000 == 0:
                    feed_dict={x: x_test, x2: x2_test, y: y_test,layer_keep_holder: 1}
                    testsummary = sess.run(testmerge, feed_dict)
                    writer.add_summary(testsummary,i)
                    feed_dict={x: x_train[:testsize], x2: x2_train[:testsize],
                               y: y_train[:testsize], layer_keep_holder: 1}
...
#        accuracy = tf.reduce_mean(tf.cast(correct,'float'))
        accuracy = acc_op.eval({x: x_test, x2: x2_test, y: y_test, layer_keep_holder: 1})
...
def build_and_train(epsilon,beta1,beta2,learning_rate,log_folder,
                    x_train, x2_train, y_train, x_test, x2_test, y_test,x_img):
...
def main():
    x_train, x2_train, y_train, x_test, x2_test, y_test = load_and_split_data()   
